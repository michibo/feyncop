...
class HopfGraph(WeightedGraph):
    ...
    EXAMPLES::
        ...
        sage: from hopf_graph import HopfGraph
        sage: G = HopfGraph([[0,1],[1,2],[1,2],[2,3]],[1,2,2,1])
        sage: G
        G[[0,1,f],[1,2,A],[1,2,A],[2,3,f]]
        sage: G.plot()
    ...
    def is_primitive(self, dimension, ym=False):
        """
        Return ``True`` if the graph is primitive.

        EXAMPLES::

            sage: from hopf_graph import HopfGraph
            sage: G = HopfGraph([[0,1],[1,2],[1,2],[2,3]],[1,2,2,1])
            sage: G.is_primitive(4)
            True

            sage: G = HopfGraph([[0,1],[0,2],[0,3],[1,2],[1,3],[2,3]],
            ....:                [1,1,1,1,1,1])
            sage: G.is_primitive(4)
            False
        """
    ...
    def reduced_coproduct(self, dimension, ym=False):
        ...
        EXAMPLES::
            ...
            sage: from hopf_graph import HopfGraph
            sage: G = HopfGraph([[0,1],[1,2],[1,2],[2,3]],[1,2,2,1])
            sage: list(G.reduced_coproduct(4))
            []
            sage: G = HopfGraph([[0,1],[0,2],[0,3],[1,2],[1,3],[2,3]],
            ....:                [1,1,1,1,1,1])
            sage: list(G.reduced_coproduct(4))
        ...
        if not self.is_edge_2_connected:
            print("Warning: Calculation of non 1PI graph coproduct omitted:",
                  self, file=sys.stderr)
            return

        yield from self.reduced_coproduct_on_edges(dimension,
                                                   self.internal_edges_set, ym)
    ...
    def reduced_coproduct_with_residue_graph(self, dimension, ym=False):
        ...
        for sub_edges_list in self.reduced_coproduct(dimension, ym):
            sub_edges_set = frozenset(e for sub_edges in sub_edges_list
                                      for e in sub_edges)
            residue_edges = self.calc_residue_edges(self.edges_set,
                                                    sub_edges_set)
    ...
    def eval_subedges_for_reduced_coproduct(self, sub_edges, dimension,
                                            sub_edges_list, ym=False):
        """
        Helper function: Evaluates omega_D on a given subgraph and checks
        if it is superficially divergent.
        """
        for component in self.cntd_components_sub_edges(sub_edges):
            ...
            if ym:
                vtx_types = (self.get_vtx_type(v)
                             for v in self.vtcs_set_sub_edges(component))
                # ghost vertices and triple gluon vertices get an extra -1.
                denom -= sum(1 for t in vtx_types
                             if (t == (2, 2, 2)) or (t == (-3, 2, 3)))
    ...
    def reduced_coproduct_on_edges(self, dimension, edges_set, ym=False):
        """Calculate the reduced coproduct in the given dimension
            only including edges in edges_set."""

        for i in range(1, len(edges_set)):
            for sub_edges in itertools.combinations(edges_set, i):
                sub_edges_list = []
                if self.eval_subedges_for_reduced_coproduct(sub_edges,
                                                            dimension,
                                                            sub_edges_list,
                                                            ym):
    ...
    def sub_graph_with_legs(self, sub_edges):
        ...
        ext_edges += [(v, v, e) for v in ext_vtcs
                      for e in self.adj_edges(v, not_edges)
                      if adj_vtx_e(e, v) == v]
        ...
        sub_graph.edges = tuple(list(sub_graph.edges)
                                + [edge for edge, _ in new_edges])
        sub_graph.edge_weights = tuple(list(sub_graph.edge_weights)
                                       + [self.edge_weights[e]
                                          for edge, e in new_edges])
    ...
    def clean_of_val2_vtcs(self):
        ...
        while True:
            bad_vtcs = [v for v in self.internal_vtcs_set
                        if self.vtx_valence(v, self.edges_set) == 2
                        and (v, v) not in self.edges]
            if not bad_vtcs:
                break
            bad_vtcs = [bad_vtcs[0]]
            pre_bad_edges = {v: tuple(self.adj_edges(v, self.edges_set))
                             for v in bad_vtcs}
            bad_edges = frozenset(e for v, edges in pre_bad_edges.items()
                                  for e in edges)
            ...
            new_edges = tuple([self.edges[e]
                               for e in self.edges_set - bad_edges]
                              + [edge for edge, _ in good_edges_w])
            new_weights = tuple([self.edge_weights[e]
                                 for e in self.edges_set - bad_edges]
                                + [w for _, w in good_edges_w])
